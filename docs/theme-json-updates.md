...
- `custom-neutral-pale` `#f9f7f3`
- `custom-neutral-light` `#efeae2`
- `custom-neutral` `#e0d6c8`
- `custom-grey` `#815e4d`
- `custom-grey-dark` `#3a382f`
- `custom-black` `#1e1b15`
- `custom-accent` `#7a3b2e`
- `custom-white` `#ffffff`
- `custom-black-5` `#1e1b150d`
- `custom-white-65` `#ffffffa6`
...
| `bg-alt` | `#efeae2` | Same value, renamed from `custom-neutral-light` for semantic clarity. |
| `paper` | `#ffffff` | Pure white for cards/inputs. Renamed from `custom-white`. |
| `ink` | `#15140f` | Concept primary text. Darker and warmer than the old `#1e1b15`. |
| `ink-2` | `#3a382f` | Same value, renamed from `custom-grey-dark`. |
| `ink-3` | `#6e6a5e` | New tertiary text color. Replaces `custom-grey` (`#815e4d`), which was a different hue not in the design system. |
| `accent` | `#7a3b2e` | Same value, renamed from `custom-accent`. |
...
- `custom-black-5` (5% black) and `custom-white-65` (65% white): no concept equivalent. If a pattern needs a translucent overlay later, add it back at that point.
- `custom-neutral` (`#e0d6c8`): the design system uses translucent rules (`rule` / `rule-soft`) as dividers, not a flat tan. The flat color isn't needed.
...
**Old:** caption color set to `custom-grey-dark`. No font family, size, or transform.
...
| `color.background` | `custom-grey-dark` (`#3a382f`) | `ink` (`#15140f`) | Concept primary buttons use `--oc-ink`. The old value was actually `ink-2`. |
